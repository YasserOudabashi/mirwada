...
VOCI = [
    ...
    {
        "data": "06.09.2026 (blocchi G-H, fase 3 chiusa)",
        "lavori": "Chiusa la fase 3. Blocco G, i talenti: un vocabolario chiuso separato per i sette comportamenti che i dodici eventi non catturano (giocato di notte, distanza percorsa, ingredienti coltivati...), tredici talenti fra innati e acquisiti, un TalentTracker che li conta e un TalentSystem che li sblocca a soglia e ne applica l'effetto - un modificatore sullo StatsComponent, o un tag per le sinergie, o una chiave che un sistema legge. Gli emettitori li ho agganciati ai sistemi esistenti con due o tre righe l'uno: il raccolto del giardino, l'uso di un'abilita' prestata, i passi del giocatore, un rituale interrotto. Alla creazione personaggio si scelgono due talenti innati con delle checkbox, e la pagina identita' li elenca; nel libro c'e' la sezione talenti coi posseduti e le barre di progresso di quelli in arrivo, offuscati finche' non hai iniziato. Blocco H: pet, talenti e stanze ora espongono i loro tag come fa l'inventario, e un piccolo SynergySources li somma tutti - e' la materia prima della fase 4, non risolve niente. Poi la vertical slice, US-335: un test solo che costruisce il giardino, coltiva, sperimenta una ricetta, forgia un'arma, incastona un sigillo col suo prezzo collaterale, doma un pet, gli fa salire il legame, sblocca un talento e controlla che i tag di tutte le fonti confluiscano - tutto pescando dai dati, senza scrivere un solo id nel test. E un grep di scripts/ per i nomi dei contenuti: zero. US-336 ha aggiornato roadmap, design doc, CLAUDE.md e README, e messo nel validator un elenco dei file di dati della fase che non devono sparire. Fase 3: 35 story su 35, 498 test, il save da schema 12 a 19.",
        "problemi": "Un bug di segno nel bonus del laboratorio: la riduzione del rischio era negativa e finiva clampata a zero, il test statistico sulle aberrazioni l'ha beccato. E la vertical slice creava un giocatore finto e non lo liberava fra un test e l'altro: due nodi 'player' in scena, e test_vertical_slice della fase 2 leggeva quello sbagliato - risolto liberando ogni player pregresso in apertura. Piu' in generale, i test di pagina che aprono il libro due volte nello stesso metodo lasciano nodi mezzo-distrutti: un test, un'apertura.",
        "osservazioni": "Il verdetto e' netto: l'architettura regge. I sette sistemi di supporto compongono un ciclo di crafting completo senza una riga di codice che nomini un contenuto. Le uniche stringhe di dominio nel codice sono nomi di vocabolari chiusi - 'laboratorio', 'enemy_defeated', 'forza' - che i sistemi indirizzano per chiave, non come casi speciali. Il punto di controllo della fase 5, aggiungere i nove Pathway rimanenti coi dati e non col codice, non e' minacciato da niente di quello che ho scritto in queste due sessioni.",
    },
]
