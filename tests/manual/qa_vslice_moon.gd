extends SceneTree
## Livello B, batch 2 (006_PRD/prd-vslice-livello-b-batch-2.md) — Moon,
...
## selettore, attraversamento del passaggio verso valle_madre, raccolta/abilita'/combattimento/alchimia.
##
## I 3 ingredienti di formula_moon_9 (radice_di_valeriana,
## rugiada_notturna, fiore_di_luna) sono stati piazzati a terra in
## data/world/layouts/valle_madre.json per questo script (batch 2, VS-B2):
## non esistevano in nessun layout prima.
##
## Questa regione ha gia' un boss Mother (Seq 7, drop garantito) mai
## combattuto con input reale: non e' di Moon, ma la preferenza-al-boss del
## motore di test lo sceglie comunque - chiude gratis il gap segnalato nel
## batch 2 §2 senza uno script dedicato.
##
## Lancio:
##   Xvfb :99 -screen 0 1280x720x24 &
##   DISPLAY=:99 godot --display-driver x11 --rendering-driver opengl3 \
##     --path . --script res://tests/manual/qa_vslice_moon.gd

const Conditions := preload("res://scripts/conditions.gd")
const OUT := "/tmp/qa_vslice_moon"
const PID := "moon"
const REGIONE_TARGET := "valle_madre"
var _main: Node
var _player: Node
var _ok: bool = true
