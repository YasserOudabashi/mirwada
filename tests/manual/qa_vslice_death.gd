extends SceneTree
## Livello B, batch 2 (006_PRD/prd-vslice-livello-b-batch-2.md) — Death,
...
## selettore, attraversamento del passaggio verso marche_crepuscolo, raccolta/abilita'/combattimento/alchimia.
##
## I 3 ingredienti di formula_death_9 (terra_di_cimitero,
## dente_di_teschio, muffa_sepolcrale) sono stati piazzati a terra in
## data/world/layouts/marche_crepuscolo.json per questo script (batch 2,
## VS-B2): non esistevano in nessun layout prima.
##
## Questa regione ha gia' un boss Twilight Giant (Seq 7, drop garantito)
## mai combattuto con input reale: non e' di Death, ma la
## preferenza-al-boss del motore di test lo sceglie comunque - chiude gratis
## il gap segnalato nel batch 2 §2 senza uno script dedicato.
##
## Lancio:
##   Xvfb :99 -screen 0 1280x720x24 &
##   DISPLAY=:99 godot --display-driver x11 --rendering-driver opengl3 \
##     --path . --script res://tests/manual/qa_vslice_death.gd

const Conditions := preload("res://scripts/conditions.gd")
const OUT := "/tmp/qa_vslice_death"
const PID := "death"
const REGIONE_TARGET := "marche_crepuscolo"
var _main: Node
var _player: Node
var _ok: bool = true
