extends SceneTree
## Livello B, batch 2 (006_PRD/prd-vslice-livello-b-batch-2.md) — Darkness,
...
## selettore, resta a Mirwada (come il Twilight Giant originale), raccolta/abilita'/combattimento/alchimia.
##
## I 3 ingredienti di formula_darkness_9 (velo_di_mezzanotte,
## occhio_che_non_dorme, cenere_di_stella_morta) sono stati piazzati a terra
## in data/world/layouts/mirwada.json per questo script (batch 2, VS-B2):
## non esistevano in nessun layout prima. Nessun attraversamento: il
## personaggio resta a Mirwada, come il Twilight Giant originale.
##
## Il boss gia' presente a Mirwada (Twilight Giant, Seq 8) e' quello gia'
## verificato dallo script originale (qa_vslice.gd): qui e' solo una
## conferma aggiuntiva, non un gap nuovo da chiudere.
##
## Lancio:
##   Xvfb :99 -screen 0 1280x720x24 &
##   DISPLAY=:99 godot --display-driver x11 --rendering-driver opengl3 \
##     --path . --script res://tests/manual/qa_vslice_darkness.gd

const Conditions := preload("res://scripts/conditions.gd")
const OUT := "/tmp/qa_vslice_darkness"
const PID := "darkness"
const REGIONE_TARGET := "mirwada"
var _main: Node
var _player: Node
var _ok: bool = true
...
func _initialize() -> void:
	...
	if _ok: await _parla_con_npc("dlg_bruno", "npc_bruno", ".bruno.n1.merce", true)
