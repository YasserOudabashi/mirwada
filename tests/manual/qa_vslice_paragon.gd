extends SceneTree
## Livello B, batch 2 (006_PRD/prd-vslice-livello-b-batch-2.md) — Paragon,
...
##
## I 3 ingredienti di formula_paragon_9 (lente_graduata,
## polvere_reagente, taccuino_di_appunti) sono stati piazzati a terra in
## data/world/layouts/mirwada.json per questo script (batch 2, VS-B2): non
## esistevano in nessun layout prima. Nessun attraversamento: il personaggio
## resta a Mirwada.
##
## Nessun boss dedicato a Paragon in nessuna regione: il combattimento sceglie
## il nemico generico piu' vicino (come Mother/Door/Error nel batch 1).
##
## Lancio:
##   Xvfb :99 -screen 0 1280x720x24 &
##   DISPLAY=:99 godot --display-driver x11 --rendering-driver opengl3 \
##     --path . --script res://tests/manual/qa_vslice_paragon.gd

const Conditions := preload("res://scripts/conditions.gd")
const OUT := "/tmp/qa_vslice_paragon"
const PID := "paragon"
...
func _initialize() -> void:
	...
	if _ok: await _parla_con_npc("dlg_vesna", "npc_vesna", ".vesna.n1.rassicura", false)
