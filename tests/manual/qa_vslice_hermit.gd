extends SceneTree
## Livello B, batch 2 (006_PRD/prd-vslice-livello-b-batch-2.md) — Hermit,
...
## selettore, attraversamento del passaggio verso archivio_sepolto, raccolta/abilita'/combattimento/alchimia.
##
## I 3 ingredienti di formula_hermit_9 (gesso_consacrato,
## polvere_di_sale_nero, filo_di_lana_rossa) erano gia' piazzati a terra in
## data/world/layouts/archivio_sepolto.json dalla fase 8 originale: zero dati
## nuovi per Hermit (scoperto nel batch 1 mentre si testava Error nella
## stessa regione).
##
## Il boss di questa regione (Seq 6, drop garantito) e' proprio di Hermit:
## il passo di combattimento lo combatte per davvero grazie alla
## preferenza-al-boss del motore di test (batch 2 §2/§3).
##
## Lancio:
##   Xvfb :99 -screen 0 1280x720x24 &
##   DISPLAY=:99 godot --display-driver x11 --rendering-driver opengl3 \
##     --path . --script res://tests/manual/qa_vslice_hermit.gd

const Conditions := preload("res://scripts/conditions.gd")
const OUT := "/tmp/qa_vslice_hermit"
const PID := "hermit"
...
func _initialize() -> void:
	...
	if _ok: await _parla_con_npc("dlg_mirco", "npc_mirco", ".mirco.n1.a", false)
