...
func test_curse_marca_e_scade_ed_e_purificabile() -> void:
	...
	e.call("_p_curse",
		...
		c, s, "ab")
	assert_true(s.call("has_modifier", "curse:ab"), "maledizione come marcatore per id")

	# light_purify la toglie (e' marcata debuff)
	...
	assert_false(s.call("has_modifier", "curse:ab"), "maledizione rimossa dalla purifica")
	_cleanup(c)
...
func test_curse_scade_da_sola() -> void:
	...
	assert_false(s.call("has_modifier", "curse:ab"), "maledizione scaduta dopo la durata")
	_cleanup(c)
